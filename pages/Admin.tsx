...
import { 
  ...
  CheckSquare, Square, Palette, Sparkles, Download
} from 'lucide-react';
...
const Admin: React.FC<AdminProps> = ({ members, setMembers, experiences, setExperiences, onAddNotification, reports, setReports }) => {
  const [activeSubTab, setActiveSubTab] = useState<'crm' | 'experiences' | 'broadcast' | 'moderation' | 'assets'>('crm');
  ...
  const [assetPrompt, setAssetPrompt] = useState('A high-end, professional logo for a South African rugby fan app called "BOKFONTEIN". The design should feature a minimalist, powerful Springbok leaping over a rugby ball. Colors: Deep Springbok Green (#004d3d) and Gold (#fdb913). Style: Modern, vector, flat, clean lines, suitable for a mobile app icon. Centered on a clean white background.');
  ...
  const handleGenerateLogo = async () => {
    ...
    native.hapticImpact();
    try {
      const imageUrl = await gemini.generateImagen(assetPrompt);
      setGeneratedLogo(imageUrl);
      onAddNotification("Asset Generated", "Official BOKFONTEIN logo is ready, boet!", "system");
      native.hapticSuccess();
    } catch (error) {
      onAddNotification("Generation Failed", "Eish! The Imagen tool is having a moment. Try again.", "system");
    } finally {
  ...
  return (
    <div className="flex flex-col h-full animate-fadeIn bg-slate-50">
      ...
      <div className="px-6 py-4 flex space-x-2 overflow-x-auto no-scrollbar bg-white border-b border-slate-100">
        ...
        <SubNavButton icon={<Palette size={16} />} label="Assets" active={activeSubTab === 'assets'} onClick={() => setActiveSubTab('assets')} />
        <SubNavButton 
          icon={<Flag size={16} />} 
          label="Safety" 
              ...
              <div className="fixed bottom-24 left-1/2 -translate-x-1/2 w-[90%] glass-premium rounded-[32px] p-4 flex items-center justify-between shadow-luxury border border-[#004d3d]/10 animate-slideUp z-[100]">
                ...
                <div className="flex items-center space-x-2">
                   <button 
                    ...
                    className="h-10 px-4 bg-geen-500 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center space-x-2"
                   >
                     <MessageSquare size={14} /> <span>WhatsApp</span>
                   </button>
                   <button 
                    onClick={() => { setDirectMsgForm(prev => ({ ...prev, channel: 'email' })); setIsMessageModalOpen(true); }}
                    className="h-10 px-4 bg-[#004d3d] text-[#fdb913] rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center space-x-2"
                   >
          ...
          <div className="space-y-8 animate-fadeIn">
            <div className="bg-[#004d3d] rounded-[40px] p-8 text-white relative overflow-hidden shadow-2xl">
              ...
              <div className="flex items-center space-x-3 mb-2">
                <Palette size={20} className="text-[#fdb913]" />
                <h3 className="text-xl font-black font-heading">Brand Asset Generator</h3>
              </div>
              <p className="text-white/60 text-xs font-medium">Use the Imagen tool to generate official BOKFONTEIN logos and visuals.</p>
            </div>
            ...
            <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm space-y-6">
              <div className="space-y-2">
                ...
                <textarea 
                  ...
                  className="w-full bg-slate-50 border border-slate-100 rounded-3xl p-6 text-sm font-medium focus:ring-2 focus:ring-[#004d3d]/5 min-h-[120px] outline-none"
                />
              </div>
              ...
              <button 
                ...
                className="w-full py-5 bg-[#004d3d] text-[#fdb913] rounded-3xl font-black uppercase tracking-widest shadow-xl flex items-center justify-center space-x-3 active:scale-95 transition-all disabled:opacity-50"
              >
                {isGeneratingAsset ? <RefreshCw className="animate-spin" size={20} /> : <Sparkles size={20} />}
                <span>{isGeneratingAsset ? 'Generating Asset...' : 'Generate with Imagen'}</span>
              </button>

              {generatedLogo && (
                <div className="pt-6 border-t border-slate-100 animate-scaleIn">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Generated Result</p>
                  <div className="relative aspect-square rounded-[32px] overflow-hidden border-4 border-slate-50 shadow-soft">
                    <img src={generatedLogo} className="w-full h-full object-cover" alt="Generated Logo" />
                    <div className="absolute bottom-4 right-4 flex space-x-2">
                      <button 
                        onClick={() => { const link = document.createElement('a'); link.href = generatedLogo; link.download = 'bokfontein-logo.jpg'; link.click(); }}
                        className="p-3 bg-white text-[#004d3d] rounded-2xl shadow-lg active:scale-90 transition-all"
                      >
                        <Download size={18} />
                      </button>
                    </div>
      ...
      {/* Experience Modal - Refined for visibility with fixed footer action area */}
      {showExpModal && (
        <div className="fixed inset-0 z-[10000] bg-black/60 backdrop-blur-md flex items-center justify-center p-4 sm:p-6 animate-fadeIn">
          <div className="bg-white w-full max-w-sm rounded-[48px] shadow-luxury animate-scaleIn relative overflow-hidden flex flex-col max-h-[85vh]">
            ...
            {/* Scrollable Form Header & Content */}
            <div className="flex-1 overflow-y-auto p-8 sm:p-10 no-scrollbar">
              ...
              <div className="space-y-6">
                ...
                <div className="h-10" /> {/* Inner Spacer */}
              </div>
            </div>

            {/* Non-Scrollable Action Footer - Guaranteed Visible */}
            <div className="p-8 pt-4 pb-24 bg-white border-t border-slate-100 shrink-0">
              <button 
                onClick={handleSaveExp}
                className="w-full py-5 bg-[#004d3d] text-white rounded-3xl font-black uppercase text-xs tracking-[0.15em] shadow-xl flex items-center justify-center space-x-2 active:scale-95 transition-all"
              >
                <Save size={18} />
